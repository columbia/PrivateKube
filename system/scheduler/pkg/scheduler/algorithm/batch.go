...
func (batch *Batch) BatchConsume(requestHandler framework.RequestHandler) bool {
	...
	if policy.AllOrNothing && !batch.tryBatchConsume(requestViewer) {
		...
		return batch.BatchRollbackByKey(keys, requestHandler, errors.AllOrNothingError(nil).Error())
	}
	...
	consumeStrategy := func(block *columbiav1.PrivateDataBlock) error {
		result = batch.core.Consume(requestViewer, block)
		if result.Error == "" {
			return nil
		} else {
	...
	for blockId := range consumeRequest.Consume {
		blockHandler := batch.cache.GetBlock(blockId)
		err := batch.updater.ApplyOperationToDataBlock(consumeStrategy, blockHandler)
		...
		if policy.AllOrNothing {
			...
			keys := util.GetStringKeysFromMap(consumeRequest.Consume)
			return batch.BatchRollbackByKey(keys, requestHandler, errors.AllOrNothingError(err).Error())
		}
	}
...
func (batch *Batch) BatchRelease(requestHandler framework.RequestHandler) bool {
	releaseRequest := requestHandler.GetRequest().ReleaseRequest
	requestViewer := requestHandler.Viewer()

	var result columbiav1.BudgetErrorPair
	releaseStrategy := func(block *columbiav1.PrivateDataBlock) error {
		result = batch.core.Release(requestViewer, block)
		if result.Error != "" {
	...
	for blockId := range releaseRequest.Release {
		...
		err := batch.updater.ApplyOperationToDataBlock(releaseStrategy, blockState)
		responseMap[blockId] = result
...
func (batch *Batch) getAvailableBlocks(requestViewer framework.RequestViewer) []*cache.BlockState {
	...
	return batch.cache.FilterBlock(predicate)
}

func (batch *Batch) tryBatchConsume(requestViewer framework.RequestViewer) bool {
	consumptionRequest := requestViewer.View().ConsumeRequest

	for blockId := range consumptionRequest.Consume {
		block := batch.cache.GetBlock(blockId).Snapshot()
		result := batch.core.Consume(requestViewer, block)
		if result.Error != "" {
			return false
		}
